...
std::unique_ptr<FaceDetectorDNN> build_detector(DetectorType type, const cv::String& xml, const cv::String& bin) {
    if (type == DetectorType::face_detection_retail_0004 ) {
        return std::make_unique<FaceDetectorDNN>(xml, bin, 0.7, 384, 672);
    } else if (type == DetectorType::face_detection_retail_0001 ){
        return std::make_unique<FaceDetectorDNN>(xml, bin, 0.5, 300, 300);
    } else {
